...
| [`restrict-manifestwork-pod-security`](restrict-manifestwork-pod-security.yaml) | high | Embedded workloads that miss a Restricted Pod Security baseline: root, privilege escalation, undropped capabilities, missing seccomp, mounted service-account tokens, `hostPort`, `procMount: Unmasked`, unsafe sysctls, and projected sources outside `configMap`/`downwardAPI`. |
