...
**Verdict: 🟢 PASSED (1 expected-unavailable)** &nbsp;·&nbsp; generated 2026-08-30 00:50:43 &nbsp;·&nbsp; local kind fleet via podman, Open Cluster Management hub
...
cluster1   true           https://cluster1-control-plane:6443   True     True        88s
cluster2   true           https://cluster2-control-plane:6443   True     True        65s
```
...
```
```
proposal_id=80bef5e87f4f42bdbdb46564e56fd4aa
{
  "status": "applied",
  "cluster": "cluster1",
...
```
{
  ...
  "unhealthy_pods": [
    {
      "namespace": "shop",
      "name": "payments-5555cb47bc-hhs8l",
      "phase": "Pending",
      "waiting": [
        "ContainerCreating"
      ],
    ...
    {
      "namespace": "shop",
      "name": "payments-5555cb47bc-lsxqb",
      "phase": "Pending",
      "waiting": [
        "ContainerCreating"
      ],
...
[
  {
    ...
    "object": "Endpoints/payments",
    "count": 1,
    "message": "Failed to create endpoint for service shop/payments: endpoints \"payments\" already exists"
  },
  {
    "namespace": "shop",
    "type": "Normal",
    "reason": "Scheduled",
    "object": "Pod/payments-5555cb47bc-hhs8l",
    "count": 1,
    "message": "Successfully assigned shop/payments-5555cb47bc-hhs8l to cluster1-control-plane"
  },
  {
    "namespace": "shop",
    "type": "Normal",
  ...
  },
  {
    ...
    "type": "Normal",
    "reason": "Scheduled",
    "object": "Pod/payments-5555cb47bc-lsxqb",
    "count": 1,
    "message": "Successfully assigned shop/payments-5555cb47bc-lsxqb to cluster1-control-plane"
  },
  {
    "namespace": "shop",
    "type": "Normal",
...
### ✅ `OK` list_applied_manifestworks(cluster1)

_What the cluster ITSELF recorded applying - the spoke's own answer, not the hub's._

```
$ tool: list_applied_manifestworks
```
```
[
  {
    "name": "43967cf4fa7b6c9c1f4014eb104077ac73d86aabc3d2c4ae6c51babdf5898540-demo-mwrspw5fr",
    "manifestwork": "demo-mwrspw5fr",
    "hub_host_hash": "",
    "applied_resources": [
      "configmaps/e2e-seed (shop)"
    ],
    "applied_count": 1
  },
  {
    "name": "43967cf4fa7b6c9c1f4014eb104077ac73d86aabc3d2c4ae6c51babdf5898540-e2e-seed",
    "manifestwork": "e2e-seed",
    "hub_host_hash": "",
    "applied_resources": [
      "configmaps/e2e-seed (shop)"
...
### ⚠️ `UNAVAILABLE` list_cluster_permissions(cluster1)

_RBAC distributed to this cluster - where to look when layer 4 refuses an apply._

```
$ tool: list_cluster_permissions
```
```
ClusterPermission CRD is not installed on this hub. It ships from open-cluster-management-io/cluster-permission, separately from core OCM.
(feature-detected: this API is not installed on a plain kind/OCM hub - it needs ACM/MCE or HyperShift. Not an error.)
```
...
```
{
  ...
  "kind": "ManagedCluster",
  "metadata": {
    "creationTimestamp": "2026-08-29T19:17:51Z",
    "finalizers": [
...
```
```
proposal_id=19482601a28b4702bd7ce462139cb5a0
...
```
$ ocm-mcp approve 19482601a28b4702bd7ce462139cb5a0
```
```
token minted (...U7pyGqSPPWAg)
```
...
```
{
  "rollback_proposal_id": "40d8566c3a4d4e98a7e0ad7e631e4f3f",
  "status": "pending_approval",
  "next_step": "Ask the human operator to run: ocm-mcp approve 40d8566c3a4d4e98a7e0ad7e631e4f3f and give you the token, then call rollback_manifestwork(40d8566c3a4d4e98a7e0ad7e631e4f3f, <token>)."
}
```
...
```
{
  "proposal_id": "4d4736931e424045b02b5293694a136f",
  "status": "pending_approval",
  "action": "cordon",
  "next_step": "Ask the human operator to run: ocm-mcp approve 4d4736931e424045b02b5293694a136f and provide you the approval token, then call apply_cluster_action."
}
```
...
```
```
taints=[{"effect": "NoSelect", "key": "ocm-mcp-server.io/cordoned", "timeAdded": "2026-08-29T19:19:47Z", "value": "true"}]
```
...
```
[
{"tool": "apply_cluster_action", "args": {"proposal_id": "7a73f81cbeec475f96295795e107ab04", "approval_token": "<redacted>"}, "outcome": "ok", "error": "", "duration_ms": 6, "ts": 1788031187.599024, "actor": "sandeepbazar:52481", "seq": 15, "prev": "97a6fd83d1792e6c19db16f819bdca25b2c4f2230a9ba812c7e87e68a21653db", "hash": "dd10ac8241179172c5671b2a719957538765fe8f32c55f50166eb0d27ea13808"},
{"tool": "propose_cluster_action", "args": {"cluster": "cluster1", "action": "accept", "summary": "accept (gated, idempotent)", "params_json": "{}"}, "outcome": "ok", "error": "", "duration_ms": 7, "ts": 1788031187.608764, "actor": "sandeepbazar:52481", "seq": 16, "prev": "dd10ac8241179172c5671b2a719957538765fe8f32c55f50166eb0d27ea13808", "hash": "522d9f083aa2ec43420bc91248309f6ccd41ab388f5ed0ba05cafd9c5b788267"},
{"tool": "apply_cluster_action", "args": {"proposal_id": "905c054544254cdf85069d20e111bdf8", "approval_token": "<redacted>"}, "outcome": "ok", "error": "", "duration_ms": 7, "ts": 1788031187.615896, "actor": "sandeepbazar:52481", "seq": 17, "prev": "522d9f083aa2ec43420bc91248309f6ccd41ab388f5ed0ba05cafd9c5b788267", "hash": "6ff0cdfe2280f02d3d6fb07c4a831a041479d1e18b19cd0a09f883bd481bcfb8"},
{"tool": "propose_cluster_action", "args": {"cluster": "cluster1", "action": "enable_addon", "summary": "enable_addon (gated)", "params_json": "{\"addon\": \"e2e-demo-addon\"}"}, "outcome": "o
... (truncated)
```
...
_Spawn the real server binary, complete the MCP handshake, and verify the wire advertises exactly what the server registered - every tool with its safety annotations, every prompt and every resource - plus a tool call, a resource read, and a prompt over the wire._
...
```
```
REJECTED: proposal df0164fd4bd54e55ab323aa9506724c8 is 'applied', not pending.
```
...
```
{
  "unhealthy_pods": [
    {
      "namespace": "shop",
      "name": "payments-v2-6f6596d897-97jxx",
      "phase": "Pending",
      "waiting": [
        "ErrImagePull"
      ],
    ...
    {
      "namespace": "shop",
      "name": "payments-v2-6f6596d897-v9qhj",
      "phase": "Pending",
      "waiting": [
        "ErrImagePull"
      ],
...
[
  {
    ...
    "type": "Normal",
    "reason": "BackOff",
    "object": "Pod/payments-v2-6f6596d897-97jxx",
    "count": 1,
  ...
  },
  {
    ...
    "type": "Warning",
    "reason": "Failed",
    "object": "Pod/payments-v2-6f6596d897-v9qhj",
    "count": 1,
...
```
```
proposal_id=d7299b682ffa4a38b1d2ed3ebf91cbae
