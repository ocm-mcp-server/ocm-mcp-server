...
def main():
    ...
    reads = [
        ...
        (f"list_applied_manifestworks({cl})",
         "What the cluster ITSELF recorded applying - the spoke's own answer, not the hub's.",
         lambda: ocm.list_applied_manifestworks(cl),
         "Nothing applied to this cluster yet; this fills in once the gated write lands."),
        (f"list_cluster_permissions({cl})",
         "RBAC distributed to this cluster - where to look when layer 4 refuses an apply.",
         lambda: ocm.list_cluster_permissions(cl),
         ("ClusterPermission ships from open-cluster-management-io/cluster-permission, "
          "separately from core OCM, so a plain kind hub does not have the CRD.")),
