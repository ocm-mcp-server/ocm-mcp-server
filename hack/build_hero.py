...
def hero(name: str) -> str:
  ...
  {wires}
  <!-- Out of the guardrail and into the outcome. Without this the eye arrives at the shield and
       stops, which is where the argument would end if the server never answered. -->
  <path class="wire" d="M492 142 H 584" fill="none" stroke="{OK}" stroke-width="1.8" opacity=".55" stroke-dasharray="5 6"/>
  <circle r="3.4" fill="{OK}"><animateMotion dur="1.8s" repeatCount="indefinite" path="M492 142 H 584"/></circle>
  <path d="M578 137 l7 5 -7 5" fill="none" stroke="{OK}" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" opacity=".8"/>
