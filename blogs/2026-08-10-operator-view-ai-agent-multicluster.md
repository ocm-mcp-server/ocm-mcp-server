...
But once the guardrails exist, someone still has to sit in the operator's chair. That person is not building the agent. They are running a fleet — today 12 clusters, next quarter probably 40 — and the question they are asking is much more concrete: **does this thing actually make my job easier, or does it just add a new thing I have to watch?**

This is that story.

I run a mixed fleet: production OpenShift clusters on-prem, a handful of ROSA clusters in AWS, and several development clusters on bare-metal kind nodes that the platform team spins up for testing. Open Cluster Management (OCM) sits as the hub layer, and I recently started routing agent requests through [ocm-mcp-server](https://github.com/sandeepbazar/ocm-mcp-server) — an MCP server that gives AI assistants a read/write interface to OCM, with every write gated behind static guardrails, a Kyverno dry-run, and a human approval token that only I can sign.

What follows is not a pitch. It is a log of what the operator's side of that arrangement actually looks like.

---

## The thing that changed first: the morning briefing

Before: I open a terminal, run `clusteradm get clusters`, look at the output, grep for anything that looks wrong, open the hub dashboard, cross-reference alerts. This takes about fifteen minutes and still misses things, because "looks wrong" is a fuzzy criterion and my eyes are imperfect.
...
That might sound small. It is not. When you run forty clusters and get paged at 3am, the difference between "fifteen minutes to understand the blast radius" and "forty seconds" is consequential.

---

## Placement: the question I used to answer by hand

OCM's placement API is elegant in theory. In practice, the loop of "write a Placement, check which PlacementDecision it produced, notice it selected the wrong clusters, figure out why, adjust the label selectors or tolerations, repeat" used to eat an hour.

A typical session now:

> **Me:** Which clusters does the `production-eu` placement currently select?
>
> **Agent:** The `production-eu` placement selects 3 clusters: `ocp-frankfurt-01`, `ocp-paris-02`, `rosa-dublin-01`. All three meet the `region=eu` label and the `environment=production` label requirements. `ocp-amsterdam-03` is excluded because it is cordoned — taint `ocm-mcp-server.io/cordoned` was applied 4 days ago.
>
...
Six weeks in, a deployment on `ocp-frankfurt-01` rolled back unexpectedly. My first instinct was to check whether the agent had done something — this is a reasonable first instinct when you have introduced a new actor into your environment.

I ran `ocm-mcp get-audit-trail --last 50`. The output is a hash-chained JSON log: every tool call, with the arguments, the outcome, the actor (the process identity running the server), and the timestamp. Each entry carries the SHA-256 hash of the previous entry, so you can verify the chain has not been tampered with.
...
What struck me was not the exoneration. It was how quickly I had a definitive answer. The audit log is not a nice-to-have compliance artifact; it is the primary debugging tool for "what did the agent do, and when." The hash chain means I cannot gaslight myself into thinking an entry was modified. The chain head can be cryptographically anchored with `ocm-mcp audit-anchor`, so even wholesale rewrites of the log are detectable.
...
**HyperShift NodePools are first-class.** If you run HCP spokes, `list_hosted_clusters` and `list_node_pools` surface the full HyperShift object graph. I use this to monitor the replica counts and upgrade status of NodePools across control-plane hosting clusters. The agent can describe a degraded NodePool and the HostedCluster conditions in one call — something that previously required cross-referencing three separate `oc get` outputs.
...
## The question I hear most: what about the Dependabot effect?

Running any server in a real environment means dealing with dependency updates. Since I started, Dependabot has proposed 6 dependency bumps to the server's own CI workflows. All of them were automatically verified with commit-SHA pins — the pre-push hook enforces this — and the four I merged passed the full CI suite: lint, mypy, 387 unit tests at 100% coverage, Kyverno policy tests, and a guardrail-parity contract that verifies the Python static layer and the Kyverno layer give identical verdicts on the same test corpus.

What this means operationally: the server I am running has a predictable, verifiable supply chain. The SBOM and Cosign signature on every release image mean I can verify the provenance of what is deployed on the hub. For an operator, that is not an abstract security property — it is the thing that keeps the security team from telling you to rip it out.
...
Before I started using the agent, my maintenance workflow was: update a label, update a placement, wait, verify. Now it is: ask the agent to propose a cordon, approve it with `ocm-mcp approve`, verify the Placement excluded the cluster, do the maintenance, ask the agent to propose the uncordon, approve again. Each step has a record. The audit log entry for the cordon includes my process identity and the stated reason. When someone asks "why was Frankfurt out of rotation for four days," the answer is one `get-audit-trail` call away.

**3. Policy compliance surfaced problems I did not know existed.**

The `list_policy_violations` tool reads from the OCM governance policy add-on. On my second day of use, it surfaced three non-compliant policy-cluster pairs I had not noticed in the dashboard. Two were stale label mismatches from a cluster rename three months ago. One was a genuine misconfiguration in a NetworkPolicy. I had not caught either in the dashboard because my usual dashboarding workflow was tuned to look at workload health, not policy compliance. The agent looks at both.

---

## What it does not do, and why that is the right call

The server deliberately does not expose `kubectl exec`, `kubectl logs` from arbitrary namespaces, or `Secret` reads. There is no tool that can pull a running pod's environment variables, read a mounted kubeconfig, or exec a shell command on a node.

For an operator who is used to having all of these things in their terminal, this feels like a constraint. It is. But the constraint is load-bearing: the agent's value in this context comes entirely from the fact that you can hand it to a model without lying awake wondering if it will `kubectl exec` its way into a secret mount and exfiltrate a database password.
...
The full [demo](https://github.com/sandeepbazar/ocm-mcp-server/tree/main/demo) shows a real unedited `e2e-local.sh` run — including the refused writes and the approved ones — if you want to see the approval flow end to end before running it yourself.
